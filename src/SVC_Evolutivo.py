# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 20:15:42 2024
...
from sklearn.model_selection import TimeSeriesSplit, cross_val_score, cross_val_predict, GridSearchCV, KFold
...
for run in range(0, n_runs):
    ...
    for dataset in datasets:
        ...
        for tk, tn in enumerate(dataset['target_names']):
            ...
            #------------------------------------------------------------------
            lb  = [1.00]*n_features  + [ 0,        1e-5,     1e+0, ]
            ub  = [1.00]*n_features  + [ 2,        1e3,      1e+4, ]
            #------------------------------------------------------------------ 
            ...
            kernel={ 0:'linear', 1:'rbf', 2:'sigmoid', 3:'poly'}
            ...
            for strategy in strategy_list[:1]:
                ...
                def objective_function(x,*args):
                    ...
                    model = SVC(C=int(round(x[-1])), gamma=x[-2], kernel=kernel[int(round(x[-3]))], random_state=random_seed, max_iter=1000)
                    scoring=make_scorer(f1_score, average='weighted', greater_is_better=True)
                    #model=GridSearchCV(estimator=model, param_grid={}, scoring=scoring, verbose=0)
                    try:
                ...
                model = SVC(C=int(round(z[-1])), gamma=z[-2], kernel=kernel[int(round(z[-3]))], random_state=random_seed, max_iter=1000)
