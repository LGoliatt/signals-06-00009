# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 21:02:42 2024
...
for run in range(0, n_runs):
    ...
    for dataset in datasets:
        ...
        for tk, tn in enumerate(dataset['target_names']):
            ...
            #------------------------------------------------------------------
            #                         solver, alpha, layers, neurons,   af,
            lb  = [1.00]*n_features  + [  0,    0.0001, 1,         1,     0, ]
            ub  = [1.00]*n_features  + [  2,     1,    2,       100,     3, ]
            #------------------------------------------------------------------ 
            ...
            af={0:'identity', 1:'logistic', 2:'tanh', 3:'relu'}
            solver={0:'adam', 1:'lbfgs', 2:'sgd'}
            ...
            for strategy in strategy_list[:1]:
                ...
                def objective_function(x,*args):
                    ...
                    model=MLPClassifier(hidden_layer_sizes = (int(round(x[-2])),)*int(round(x[-3])),
                                       activation=af[int(round(x[-1]))],
                                        solver=solver[int(round(x[-5]))],
                                       random_state=random_seed, validation_fraction=0,
                                        # solver='lbfgs',
                                       alpha = x[-4],
                                       max_iter=1000)
                    try:
                ...
                model=MLPClassifier(hidden_layer_sizes = (int(round(z[-2])),)*int(round(z[-3])),
                                   activation=af[int(round(z[-1]))],
                                    solver=solver[int(round(z[-5]))],
                                   random_state=random_seed, validation_fraction=0,
                                    # solver='lbfgs', 
                                   alpha = z[-4],
                                   max_iter=1000)
